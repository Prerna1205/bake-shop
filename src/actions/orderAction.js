...
export const newOrder = (order,user,token) => async (dispatch) => {
    ...
    try {
        ...
        const { data } = await axios.post(`${apiBaseUrl}/api/orders/new`, bodyParameters, config);
...
export const myOrders = (token) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`${apiBaseUrl}/api/orders/me`,config);
...
export const getPaymentStatus = (id,user,token) => async (dispatch) => {
    ...
    try {
        ...
        const { data } = await axios.get(`${apiBaseUrl}/api/payment/status/${id}`,bodyParameters,config);
